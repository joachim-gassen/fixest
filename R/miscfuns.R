...
collinearity = function(x, verbose){
  ...
  if(isLinear){
    ...
    varkeep = intersect(c(names(x$coefficients), x$collin.var), colnames(linear.matrix))
  ...
  if(isSlope){
    ...
    if(any(!slope_vars_unik %in% names(data))){
      ...
      stopi("To check collinearity, we need to fetch some variables in the original dataset (",
           deparse_long(x$call$data), "). ",
           "However, the variable {$s, enum, is?var_pblm} not present in the original dataset any more.")
    }
...
#'
#'
#' @param fml Either a formula of the type `var1 + ... + varN ~ treat` or 
#' `var1 + ... + varN ~ treat | post`. Either a data.frame/matrix containing 
...
#' @param treat_var Only if argument `fml` is *not* a formula. The vector identifying 
...
#' @param post_var Only if argument `fml` is *not* a formula. The vector identifying 
...
#' @param treat_first Which value of the 'treatment' vector should appear on the left? 
#' By default the max value appears first (e.g. if the treatment variable is a 0/1 vector, 
#' 1 appears first).
#' @param tex Should the result be displayed in Latex? Default is `FALSE`. Automatically 
#' set to `TRUE` if the table is to be saved in a file using the argument `file`.
#' @param treat_dict A character vector of length two. What are the names of the treated 
#' and the control? This should be a dictionary: e.g. `c("1"="Treated", "0" = "Control")`.
#' @param dict A named character vector. A dictionary between the variables names and an alias. 
#' For instance `dict=c("x"="Inflation Rate")` would replace the variable name `x` 
#' by \dQuote{Inflation Rate}.
#' @param file A file path. If given, the table is written in Latex into this file.
#' @param replace Default is `TRUE`, which means that when the table is exported, the 
...
#' @param indiv Either the variable name of individual identifiers, a one sided formula, 
#' or a vector. If the data is that of a panel, this can be used to track the number 
#' of individuals per group.
#' @param prepostnames Only if there is a 'post' variable. The names of the pre and post 
...
did_means = function(fml, base, treat_var, post_var, tex = FALSE, treat_dict,
                     ...
                     prepostnames = c("Before", "After"), diff.inv = FALSE){
  # x is a data.frame
  ...
  if(!usePost){
    # the simple case
    res = .meanDiffTable(mat_vars, treat_var, treat_cases, indiv_var, raw = raw, 
                         diff.inv = diff.inv)
    ...
    res_pre = .meanDiffTable(mat_vars[qui_pre, ], treat_var[qui_pre], treat_cases, 
                             indiv_var[qui_pre], raw = raw, diff.inv = diff.inv)

    qui_post = which(post_var == post_values[2])
    res_post = .meanDiffTable(mat_vars[qui_post, ], treat_var[qui_post], treat_cases, 
                              indiv_var[qui_post], raw = raw, diff.inv = diff.inv)
  ...
  if(tex || !missing(file)){
    ...
    if(!usePost){
      ...
      # the treat post line
      cat(" & ", "\\multicolumn{4}{c}{", prepostnames[1], "} & \\multicolumn{4}{c}{", 
          prepostnames[2], "}\\tabularnewline\n")
...
#' @inheritParams bin
#'
#' @param factor_var  A vector (of any type) that will be treated as a factor. 
#' You can set references (i.e. exclude values for which to create dummies) with 
#' the `ref` argument.
#' @param var A variable of the same length as `factor_var`. This variable will be 
...
#' @param ref A vector of values to be taken as references from `factor_var`. 
...
#' @param keep A vector of values to be kept from `factor_var` (all others are dropped). 
...
#' @param ref2 A vector of values to be dropped from `var`. By default they 
...
#' @param keep2 A vector of values to be kept from `var` (all others are dropped). 
...
#' to enable regular expression matching instead.
#' @param bin2 A list or vector defining the binning of the second variable.
...
#' `setFixest_fml(..ctrl = ~Temp | Day)`, but then `feols(Ozone ~ Wind + ..ctrl, airquality)` 
#' will be quite different from `feols(Ozone ~ ..ctrl + Wind, airquality)`, so beware!
#'
#'
...
.xpd = function(fml, ..., add = NULL, lhs = NULL, rhs = NULL, 
  ...
  if(is_macro){
    ...
    if(depth == max_depth && length(qui) > 0){
      warni("In xpd, max recursivity has been reached. ",
            "Please revise the recursive definition of your variables. ",
            "It concerns the variable{$s, q, enum ? names(macros)[qui]}.")
    }
  }
...
#' to a formula, then the argument `data` is required, and it must be of the type: 
#' `x1 + x2 ~ f1 + fe2` with on the LHS the variables to be centered, and on the RHS the factors 
#' used for centering. Note that you can use variables with varying slopes with the syntax 
#' `fe[v1, v2]` (see details in [`feols`]). If a `feols` estimation, all variables (LHS+RHS) are 
...
#' same as the factors used for centering (argument `f`).
#' @param f A matrix, vector, data.frame or list. The factors used to center the variables in 
#' argument `X`. Matrices will be coerced using `as.data.frame`.
#' @param slope.vars A vector, matrix or list representing the variables with varying slopes. 
#' Matrices will be coerced using `as.data.frame`. Note that if this argument is used it MUST be in 
#' conjunction with the argument `slope.flag` that maps the factors to which the varying slopes are 
#' attached. See examples.
#' @param slope.flag An integer vector of the same length as the number of variables in `f` (the 
...
#' but see the examples it may get clearer.
#' @param data A data.frame containing all variables in the argument `X`. Only used if `X` is a 
...
#' @param tol Stopping criterion of the algorithm. Default is `1e-6`. The algorithm stops when the 
#' maximum absolute increase in the coefficients values is lower than `tol`.
#' @param na.rm Logical, default is `TRUE`. If `TRUE` and the input data contains any NA value, 
#' then any observation with NA will be discarded leading to an output with less observations than 
#' the input. If `FALSE`, if NAs are present the output will also be filled with NAs for each NA 
#' observation in input.
#' @param as.matrix Logical, if `TRUE` a matrix is returned, if `FALSE` it will be a data.frame. 
#' The default depends on the input, if atomic then a matrix will be returned.
#' @param im_confident Logical, default is `FALSE`. FOR EXPERT USERS ONLY! This argument allows to 
#' skip some of the preprocessing of the arguments given in input. If `TRUE`, then `X` MUST be a 
#' numeric vector/matrix/list (not a formula!), `f` MUST be a list, `slope.vars` MUST be a list, 
#' `slope.vars` MUST be consistent with `slope.flag`, and `weights`, if given, MUST be numeric (not 
...
#' break.
#' @param fixef.reorder Logical, default is `TRUE`. Whether to reorder the fixed-effects by 
#' frequencies before feeding them into the algorithm. If `FALSE`, the original fixed-effects order 
...
#'
#' If `na.rm = TRUE`, then the number of rows is equal to the number of rows in input minus the 
#' number of NA values (contained in `X`, `f`, `slope.vars` or `weights`). The default is to have 
...
#' @param type Either "short" (default) or "detail". If "short", only the maximum absolute FOC are 
#' displayed, otherwise the 2 smallest and the 2 largest FOC are reported for each fixed-effect and 
#' each variable.
#' @param ... Not currently used.
#'
#' Note that this function first re-demeans the variables, thus possibly incurring some extra 
#' computation time.
#'
#' @return
...
#' @param vcov A list containing the types of standard-error to be computed, default is missing. If 
...
#' 
#' @param extraProj Integer scalar, default is 0. Should there be more plain projection steps 
...
#' @param iter_warmup Integer scalar, default is 15. Only used in the presence of 3 
#' or more fixed-effects (FE), ignored otherwise. For 3+ FEs, the algorithm is as follows:
#' 1) `iter_warmup` iterations on all FEs. If convergence: end of the algorithm. 2) Otherwise: 
...
#' 
#' @details 
#' The demeaning algorithm is a fixed-point algorithm. Basically a function `f` is applied
#' until `|f(X) - X| = 0`, i.e. there is no difference between `X` and its image. 
#' For terminology, let's call the application of `f` a "projection".
#' 
#' For well behaved problems, the algorithm in its simplest form, i.e. just applying `f` until
...
#' function `f` can lead to extremely slow convergence. To handle these cases, this algorithm
...
parse_macros = function(..., reset = FALSE, from_xpd = FALSE, check = TRUE, frame = NULL){
  ...
  if(check){
    ...
    if(any(qui_pblm)){
      ...
      stop_up("Each argument name must start with two dots. Use {q?correct_name} instead of {q?arg_name}.")
    }
  }
...
dsb_check_set_agg = function(...){
  ...
  if(...length() > 1){
    ...
    stop_up("You cannot have more than two elements in between .[]. Currently there are ", 
            ...length() + 1, ".")
  }
